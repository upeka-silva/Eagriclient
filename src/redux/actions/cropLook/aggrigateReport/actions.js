...
export const approveBiWeekCategoryReport = async (
  ...
  try {
    const response = await put(`crop-look/crop-category-report/season/${seasonId}/week/${weekId}/category/${cropCategoryId}`, null, true);
    if (response?.httpCode === "200 OK") {
      onSuccess();
      return {
        dataList: response?.payload,
...
export const getApprovalData = async (seasonId, weekId) => {
  try {
    const { httpCode, payloadDto } = await get(`crop-look/crop-category-summary/season/${seasonId}/week/${weekId}`, true);
    if (httpCode === "200 OK") {
      return payloadDto;
    }
    return {
