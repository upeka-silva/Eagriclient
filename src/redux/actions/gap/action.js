...
export const updateGap = async (
  ...
  export const handleCropDetails = async (
    ...
    try {
      ...
      const response = await post("gap-request/" + gapReqId + "/crop-areas", payload, true);
      if (response.httpCode === "200 OK") {
        onSuccess();
        console.log("Success")
      } else {
  ...
  export const getCropDetailsList = async (gapReqId = null) => {
    try {
      const { httpCode, payloadDto } = await get("gap-request/"+ gapReqId + "/crop-areas", true);
      if (httpCode === "200 OK") {
        return {
          dataList: payloadDto,
        };
  ...
  export const deleteCropDetails = async (
    ...
    try {
        const response = await api_delete(`gap-request/${gapReqId || ''}/crop-areas/${id || ''}`, true);
        if (response?.httpCode === "200 OK") {
            onSuccess();
        } else {
            const exception = {
