...
import { UpdateAgriculturePostImage, handleAgriculturePost } from "../../redux/actions/extension/action";
...
const CreatePostForm = () => {
  ...
  const enableSave = () => {
    if (state?.action === DEF_ACTIONS.EDIT) {
      if (JSON.stringify(state?.target || {}) !== JSON.stringify(formData) || selectedFile != null || selectedFile2 != null || selectedFile3 != null) {
        return true;
      }
    }
    if (
