...
const FaoEmergencyMap = (location) => {
  ...
  const [sriLankaMap, setSriLankaMap] = useState([]);

  // Define a color scale
  const colorScale = d3
    .scaleSequential()
    .interpolator(d3.interpolateGreens) // You can choose any color scale you prefer
    .domain([0, d3.max(Object.values(districtDistribution))]); // Domain based on the maximum density

  // Function to get the color based on the district's density
  const getColor = (district) => {
    const density = districtDistribution[district];
    return density ? colorScale(density) : "white"; // Assign gray if density is null or undefined
  };

  // Update the JSX elements for each district to include the color
  useEffect(() => {
    for (let district in svgPaths) {
      svgPaths[district].forEach((path) => {
        //const className = district === selectedDistrict ? "map-region active-path" : "map-region";

        let tooltipContent = <div>{district}</div>;

        let content = (
          <Tooltip key={district} title={tooltipContent} placement="top">
            <path
              key={district}
              //className={className}
              d={path}
              fill={getColor(district)} // Set the fill color based on the density
              //onMouseOver={(event) => handleRegionClick(event, district)}
            />
          </Tooltip>
        );
        setSriLankaMap((sriLankaMap) => [...sriLankaMap, content]);
      });
    }
  }, []);
  ...
  return (
    <Grid sx={{ height: "100%" }}>
      ...
      <Grid className="locationSearch row map-container">
        <Grid className="col-md-10 col-xs-8">
          <Grid className="map">
            <svg height="600" width="320" viewBox="6 0 10 40">
...
export default FaoEmergencyMap;
