...
import { get_DataList, post_DataList } from "../../redux/actions/list/list";
              ...
              <AdvancedSearchItemWrapper key={key}>
                <Autocomplete
                  ...
                  renderOption={(props, option) => (
                    <li {...props} value={option[data?.listTarget || k]}>
                      {(data?.listLabels || [])
                        .map((l) => option[l] || "")
                        .join(data?.listLabelJoint || " ")}
                    </li>
      ...
      <Table sx={{ borderCollapse: "unset !important" }} size="small">
        <TableHead>
          <TableRow >
            ...
            {selectable && (
              <TableCell
                sx={{ border: "none #CCC !important", width: "50px" }}
              >
                <ActionToolTip
                  title={
                  ...
                  if (c?.type === "list" || Array.isArray(c?.field)) {
                    return (
                      <TableCell key={`${key}-${key2}`}>
                        ...
                        {(c?.field || [])
                          .map((field) => extractNestedData(r, field) || "")
                          .join(c?.joinString || " - ")}
                      </TableCell>
                    );
                  }
...
const SelectableRow = styled(TableRow)`
  ...
  // :hover td {
  //   border-top: none ${theme.coreColors.primary} !important;
  //   border-bottom: none ${theme.coreColors.primary} !important;
  //   transition: 0.3s ease;
  // }

  // :hover td:first-child {
  //   border-left: none ${theme.coreColors.primary} !important;
  //   border-top-left-radius: 5px !important;
  //   border-bottom-left-radius: 5px !important;
  // }

  // :hover td:last-child {
  //   border-right: none ${theme.coreColors.primary} !important;
  ...
  & td {
    border: ${(props) =>
      props.selected
        ? `none ${theme.coreColors.primary}00 !important`
        : "none #CCC !important"};
    border-top: ${(props) =>
      props.selected
        ? `none ${theme.coreColors.primary} !important`
        : "none #CCC !important"};
    border-bottom: ${(props) =>
      props.selected
        ? `none ${theme.coreColors.primary} !important`
